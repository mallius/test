 ...
 * @column:	the column address for this command, -1 if none		//列地址?
 * @page_addr:	the page address for this command, -1 if none		//行地址?
 *
...
static void nand_resume(struct mtd_info *mtd)		//for nand_scan_tail
{
	struct nand_chip *chip = mtd->priv;
 ...
 */
static void nand_set_defaults(struct nand_chip *chip, int busw)	//busw = buswidth = NAND_BUSWIDTH_16
{
	...
	if (chip->cmdfunc == NULL)
		chip->cmdfunc = nand_command;			//用到很多,发命令
	...
	if (!chip->select_chip)
		chip->select_chip = nand_select_chip;		//用到很多
	if (!chip->read_byte)
						  ...
						  int busw, int *maf_id)
{
	...
	chip->cmdfunc(mtd, NAND_CMD_READID, 0x00, -1);		//读nand ID
 ...
 */
int nand_scan_ident(struct mtd_info *mtd, int maxchips)		//maxchips = 1
{
	int i, busw, nand_maf_id;
	struct nand_chip *chip = mtd->priv;		//mtd->priv指向nand_chip
	struct nand_flash_dev *type;

	/* Get buswidth to select the correct functions */
	busw = chip->options & NAND_BUSWIDTH_16; //option:用于nand_scan,buswidth=16bit
	/* Set the default functions */			//初始化chip结构体中的函数指针
...
*/
#if 1			
void nand_release(struct mtd_info *mtd)		// 没有用到
{
	struct nand_chip *chip = mtd->priv;
